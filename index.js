...
try{
    ...
    if (command === "invite" || command === "add"){
        return embedbuilder(client, message, "#c219d8", "Invite this Bot!", "[`Click here`](https://discord.com/api/oauth2/authorize?client_id=799887819949867010&permissions=8&scope=bot)   |   [`Support`](https://discord.gg/BnbkER6DYQ)   |   :heart: Thanks for inviting!")
         }
    if (command === "radio") {
    ...
    else if (command === "loop" || command === "rp") {
        if (0 <= Number(args[0]) && Number(args[0]) <= 2) {
            await distube.setRepeatMode(message, parseInt(args[0]));
            await message.guild.channels.cache.get(db.get(`playingchannel_${message.guild.id}`)).messages.cache.get(db.get(`playingembed_${message.guild.id}`), false, true).edit(curembed(message))
            await embedbuilder(client, message, "#c219d8", "Loop Mode set to:", `${args[0].replace("0", "OFF").replace("1", "Loop song").replace("2", "Loop Queue")}`)
            await delay(5000);
            await message.channel.bulkDelete(2)
            return
        }
...
function embedbuilder(client, message, color, title, description, thumbnail) {
    try{   let embed = new Discord.MessageEmbed()
        .setColor(color)
        .setAuthor(message.author.tag, message.member.user.displayAvatarURL({ dynamic: true }),"https://discord.gg/BnbkER6DYQ")
