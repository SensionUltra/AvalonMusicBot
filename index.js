...
client.on("message", async message => {
    ...
    if (command === "help") {
        ...
        Supported sources: youtube for now
        Bot by: Awoken#6969  [Server](https://discord.gg/xzrk9NPca5)  [Avalon](https://discord.com/oauth2/authorize?client_id=799887819949867010&permissions=8&scope=bot)
        `)
        return;
    }
        ...
        await searchembed.channel.awaitMessages(m => m.author.id == message.author.id, { max: 1, time: 60000, errors: ["time"], }).then(collected => {
            ...
            if (isNaN(userinput)) {
                embedbuilder(client, message, "RED", "Not a right number!", "so ill use number 1!")
                userinput = 1;
            }
